...
resource "aws_route_table" "private_internet_access_1a" {
  vpc_id = aws_vpc.main.id
  tags = {
    Name = format("%s-private-rtb-1a", var.project_name)
    Environment = var.environment
    Terraform   = "True"
  }
}
...
resource "aws_route_table" "private_internet_access_1b" {
  vpc_id = aws_vpc.main.id
  tags = {
    Name = format("%s-private-rtb-1b", var.project_name)
    Environment = var.environment
    Terraform = "True"
  }
}
...
resource "aws_route_table_association" "private_1b" {
  ...
  route_table_id = aws_route.private_access_1b.id
}
...
resource "aws_route_table" "private_internet_access_1c" {
  vpc_id = aws_vpc.main.id
  tags = {
    Name = format("%s-private-rtb-1c", var.project_name)
    Environment = var.environment
    Terraform = "True"
  }
}
...
resource "aws_route_table_association" "private_1c" {
  ...
  route_table_id = aws_route.private_access_1c.id
}
